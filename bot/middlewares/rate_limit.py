...
class RateLimitMiddleware(BaseMiddleware):
    """Ограничивает частоту скачиваний — только для текстовых сообщений с Rutube-ссылками"""
    ...
    async def __call__(
        ...
        # проверяем что текст похож на ссылку Rutube
        from bot.utils.helpers import is_rutube_url
        if not is_rutube_url(event.text.strip()):
            return await handler(event, data)
