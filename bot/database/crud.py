...
async def get_cached_download(
    session: AsyncSession, source_url: str, format_key: str
) -> Download | None:
    ...
    result = await session.execute(
        select(Download).where(
            ...
            Download.expires_at > datetime.now(timezone.utc),
        )
    )
    download = result.scalar_one_or_none()

    # обновляем счётчик если кэш найден
    if download:
...
async def save_download(
    ...
    if download:
        ...
        download.created_at = datetime.now(timezone.utc)
        download.download_count += 1
        download.expires_at = datetime.now(timezone.utc) + timedelta(days=1)
    else:
        download = Download(
...
async def cleanup_expired_downloads(session: AsyncSession) -> int:
    """Удаляет протухшие записи кэша. Возвращает количество удалённых."""
    result = await session.execute(
